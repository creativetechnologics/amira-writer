...
from beat_lighting_diff_report import report


def to_markdown(payload: dict) -> str:
    lines = ['# Beat Lighting Diff Report', '']
...
def main() -> int:
    if len(sys.argv) != 4:
        print('Usage: generate_beat_lighting_diff_bundle.py <old-beat-plan.json> <new-beat-plan.json> <out-dir>')
        return 2
    out = Path(sys.argv[3])
    out.mkdir(parents=True, exist_ok=True)
    payload = report(json.loads(Path(sys.argv[1]).read_text()), json.loads(Path(sys.argv[2]).read_text()))
    (out / 'beat_lighting_diff_report.json').write_text(json.dumps(payload, indent=2) + '\n')
    (out / 'beat_lighting_diff_report.md').write_text(to_markdown(payload))
