...
enum ConsoleAgentType: String, CaseIterable, Sendable {
    case claudeCode = "Claude Code"
    case codex = "Codex"
}

enum ConsoleAgentModel: String, CaseIterable, Sendable {
    case opus = "opus"
    case sonnet = "sonnet"
    case haiku = "haiku"
    ...
    static func available(for agent: ConsoleAgentType) -> [ConsoleAgentModel] {
        switch agent {
...
final class ScriptStore {
    ...
    var showDirections: Bool = UserDefaults.standard.object(forKey: "showDirections") as? Bool ?? false {
        didSet { UserDefaults.standard.set(showDirections, forKey: "showDirections") }
    }
    var showStoryboarding: Bool = UserDefaults.standard.object(forKey: "showStoryboarding") as? Bool ?? false {
        didSet { UserDefaults.standard.set(showStoryboarding, forKey: "showStoryboarding") }
    }
    var showAnimateDirections: Bool = UserDefaults.standard.object(forKey: "showAnimateDirections") as? Bool ?? false {
        didSet { UserDefaults.standard.set(showAnimateDirections, forKey: "showAnimateDirections") }
    }
    ...
    private var lastAutoVersionDates: [String: Date] = [:]
    private static let autoVersionInterval: TimeInterval = 300  // 5 minutes
    private static let maxAutoVersionsPerSong: Int = 50
    ...
    private var latestDatabaseChangeSequence: Int64 = 0
    ...
    private let databaseSourceID = "novotro-write"
    ...
    func loadProject(url: URL, preferService: Bool? = nil) async {
        ...
        do {
            ...
            if isStandalone {
                ...
                loaded = nil
                projectDatabase = nil
                latestDatabaseChangeSequence = 0
            } else {
                let result = try await ProjectDatabaseBridge.loadWriterProject(url: url, preferService: preferService)
                loaded = result
                meta = result.metadata
                stubs = result.stubs
                projectDatabase = result.database
                latestDatabaseChangeSequence = (try? await result.database.currentChangeToken()) ?? 0
            }
            ...
            self.projectHistoryEntries = previousHistoryState.entries
            ...
            if let loaded,
               let synopsisData = try? await ProjectDatabaseBridge.loadProjectFile(
                    ...
                    candidatePaths: [ProjectDatabaseBridge.synopsisPath]
               ),
            ...
            await MainActor.run { [weak self] in
                ...
                if let saveError {
                    ...
                    for song in songsToSave {
                        let fileURL = url.appendingPathComponent(song.relativePath)
                    ...
                    if !historyPaths.isEmpty {
                        syncSongsToDatabase(paths: Set(historyPaths))
    ...
    @discardableResult
    private func checkAutoVersion(forPath path: String) -> Bool {
        let now = Date()
        let lastDate = lastAutoVersionDates[path] ?? .distantPast
        guard now.timeIntervalSince(lastDate) >= Self.autoVersionInterval else { return false }

        return recordRevisionSnapshot(forPath: path, at: now)
    }

    @discardableResult
    private func recordRevisionSnapshot(
        forPath path: String,
        at timestamp: Date = Date(),
        historyTitle: String = "Recorded revision",
        ...
        let latestRevisionLyrics = songAssets[songIdx].document.versions
            .filter { $0.id != activeID && $0.saveType == .autosave }
            .max(by: { $0.createdAt < $1.createdAt })?
            .lyrics
        guard latestRevisionLyrics != currentLyrics else {
            lastAutoVersionDates[path] = timestamp
            return false
        }

        let newVersion = OWSVersionPayload(
            id: UUID(),
            label: "Revision \(Self.versionDateFormatter.string(from: timestamp))",
            createdAt: timestamp,
            updatedAt: timestamp,
        ...
        lastAutoVersionDates[path] = timestamp
        pruneAutoVersions(forSongAt: songIdx)
        appendProjectHistory(
            kind: .autosave,
            title: historyTitle,
            message: historyMessage ?? summarizeTrackedPaths([path]),
            relativePaths: [path]
        )
        return true
    }
    ...
    @discardableResult
    private func preserveCurrentDraftRevisionIfNeeded(
        forPath path: String,
        reason: String
    ) -> Bool {
        guard dirtySongPaths.contains(path) else { return false }
        return recordRevisionSnapshot(
            forPath: path,
            historyTitle: "Preserved local draft",
            historyMessage: "\(path) before \(reason)"
        )
    }

    private func pruneAutoVersions(forSongAt songIdx: Int) {
        let autoVersions = songAssets[songIdx].document.versions
            .filter { $0.saveType == .autosave }
            .sorted { $0.createdAt < $1.createdAt }

        if autoVersions.count > Self.maxAutoVersionsPerSong {
            let toRemove = autoVersions.prefix(autoVersions.count - Self.maxAutoVersionsPerSong)
            let removeIDs = Set(toRemove.map(\.id))
            songAssets[songIdx].document.versions.removeAll { removeIDs.contains($0.id) }
        }
    }
    ...
    func saveSynopsis() {
        ...
        do {
            ...
            if let projectDatabase, let data = synopsisText.data(using: .utf8) {
                Task {
                    try? await projectDatabase.upsertProjectFile(
                        path: ProjectDatabaseBridge.synopsisPath,
                        jsonData: data,
                        actorID: ProjectDatabaseBridge.writeActorID
                    )
                    try? await projectDatabase.refreshLegacyFingerprint()
                }
            }
        } catch {
    ...
    private var synopsisSaveWorkItem: DispatchWorkItem?

    func scheduleSynopsisSave() {
        synopsisSaveWorkItem?.cancel()
        let item = DispatchWorkItem { [weak self] in
            self?.saveSynopsis()
        }
        synopsisSaveWorkItem = item
    ...
    private func loadScratchpad(
        from projectURL: URL,
        database: NovotroProjectConnection?
    ) async {
        let loadedText: String
        if let database,
           let data = try? await ProjectDatabaseBridge.loadProjectFile(
                from: database,
                candidatePaths: [ProjectDatabaseBridge.scratchpadPath]
           ),
           let text = String(data: data, encoding: .utf8) {
            loadedText = text
        } else {
        ...
        do {
            ...
            if let projectDatabase,
               let data = scratchpadDocumentText.data(using: .utf8) {
                Task {
                    try? await projectDatabase.upsertProjectFile(
                        path: ProjectDatabaseBridge.scratchpadPath,
                        jsonData: data,
                        actorID: ProjectDatabaseBridge.writeActorID
                    )
                }
    ...
    private func ensureClaudeMD(in projectURL: URL) {
        ...
        if !FileManager.default.fileExists(atPath: parentClaudeMD.path) {
            ...
            This project uses the Novotro Write OWP package format. All project files live inside the `\(owpName)/` directory.
        ...
        This is a Novotro Write opera/musical project.
        ...
        **Scene markers:** The synopsis contains hidden navigation markers in the format:
        ...
        ```
        These markers appear on their own line before the synopsis text for that scene. They are used by Novotro Write \
        ...
        **When you modify scenes:** After making changes to any scene's lyrics/libretto, you MUST update the synopsis \
        ...
        3. **Animate instructions** — single curly braces: `{camera: zoom_in | from=wide | to=close | bars=17-24}`
           Technical instructions for Novotro Animate. Keywords: camera, enter, exit, scene, lipsync, emotion, lighting.
    ...
    func applyEditorChange(path: String, lyrics: String) {
        _ = checkAutoVersion(forPath: path)
            ...
            if lastKnown == nil {
                ...
                if let data = try? Data(contentsOf: synopsisURL),
                   ...
                   text != synopsisText {
                    ...
                    refreshGitHistory()
                    Task { [projectDatabase, data] in
                        try? await projectDatabase?.upsertProjectFile(
                            path: OWPProjectIO.synopsisFile,
                            jsonData: data,
    ...
    private func reloadExternallyChanged(stub: SongStub, modDate: Date) {
        ...
        beginAgentSync()
        _ = preserveCurrentDraftRevisionIfNeeded(forPath: path, reason: "external reload")
        let localRevisionVersions = localRevisionVersions(forPath: path)
        ...
        Task {
            do {
                ...
                mergeLocalRevisionVersions(localRevisionVersions, into: &asset.document)

                if let idx = songAssets.firstIndex(where: { $0.relativePath == path }) {
                    songAssets[idx] = asset
                    pruneAutoVersions(forSongAt: idx)
                }
    ...
    private func localRevisionVersions(forPath path: String) -> [OWSVersionPayload] {
        guard let songIdx = songAssets.firstIndex(where: { $0.relativePath == path }) else { return [] }
        return songAssets[songIdx].document.versions.filter { $0.saveType == .autosave }
    }

    private func mergeLocalRevisionVersions(_ revisions: [OWSVersionPayload], into document: inout OWSSongDocument) {
        var existingIDs = Set(document.versions.map(\.id))
        for revision in revisions where !existingIDs.contains(revision.id) {
            document.versions.append(revision)
            existingIDs.insert(revision.id)
        }
    }
    ...
    private func pollDatabaseChanges() {
        ...
        Task {
            do {
                let changes = try await projectDatabase.listChanges(since: latestDatabaseChangeSequence)
                guard !changes.isEmpty else { return }
                await MainActor.run {
                    latestDatabaseChangeSequence = changes.last?.changeID ?? latestDatabaseChangeSequence
                }
                ...
                NSLog("[ScriptStore] Database poll error: %@", error.localizedDescription)
            }
        }
    }
    ...
    private func applyDatabaseChange(_ change: ChangeEvent) {
        guard let projectDatabase else { return }
        guard change.actorID != databaseSourceID else { return }
            ...
            Task {
                ...
                await MainActor.run {
                    if let songIdx = songAssets.firstIndex(where: { $0.relativePath == relativePath }) {
                        ...
                        songAssets.append(OWSSongAsset(relativePath: relativePath, document: document))
                    }
                    if let activeLyrics = document.activeVersion()?.lyrics {
                        if let librettoIdx = librettoFiles.firstIndex(where: { $0.relativePath == relativePath }) {
                            librettoFiles[librettoIdx].content = activeLyrics
                        } else {
                            librettoFiles.append(ProjectTextFile(
                                id: UUID(),
                                relativePath: relativePath,
                                content: activeLyrics
                            ))
                        }
                    }
                    hydratedScenePaths.insert(relativePath)
                    normalizeScratchpadFiles()
                    markAgentUpdated(paths: [relativePath])
                }
                } catch {
                    NSLog("[ScriptStore] Scene load error: %@", error.localizedDescription)
                }
            }
        case "project_file" where ProjectDatabaseBridge.charactersPaths.contains(change.entityKey):
            Task {
                do {
                    let artifact = try await ProjectDatabaseBridge.loadCharactersFile(from: projectDatabase)
                    let decoded = try ProjectDatabaseBridge.decodeCharacters(from: artifact)
                    await MainActor.run {
                        characters = decoded
                    }
                } catch {
                    NSLog("[ScriptStore] Characters load error: %@", error.localizedDescription)
                }
            }
        case "project_file" where change.entityKey == ProjectDatabaseBridge.synopsisPath:
            Task {
                do {
                    let data = try await ProjectDatabaseBridge.loadProjectFile(
                    ...
                    NSLog("[ScriptStore] Synopsis load error: %@", error.localizedDescription)
                }
            }
        case "project_file" where change.entityKey == ProjectDatabaseBridge.scratchpadPath:
            Task {
                do {
                    let data = try await ProjectDatabaseBridge.loadProjectFile(
                        from: projectDatabase,
                    ...
                    NSLog("[ScriptStore] Scratchpad load error: %@", error.localizedDescription)
                }
            }
        default:
            break
    ...
    func ensureSceneHydrated(path: String) {
        ...
        Task { [weak self, database, stub, path] in
            guard let self else { return }

            let hydratedAsset = await self.hydrateScene(path: path, database: database, stub: stub)
    ...
    private func hydrateScene(
        ...
        if let database,
           let scene = try? await database.loadScene(relativePath: path) {
            return OWSSongAsset(
                relativePath: path,
                document: ProjectDatabaseBridge.makeDocument(from: scene)
            )
        }
        if let stub {
    ...
    private func syncSongTextToDatabase(path: String, lyrics: String) {
        guard let projectDatabase else { return }
        let versionID = songAssets.first(where: { $0.relativePath == path })?.document.activeVersionID
        suppressDatabaseSyncPaths.insert(path)
        let workItem = DispatchWorkItem { [weak self] in
            guard let self else { return }
            Task {
                try? await projectDatabase.updateSongText(
                    relativePath: path,
                    lyrics: lyrics,
                    versionID: versionID,
                    actorID: ProjectDatabaseBridge.writeActorID
    ...
    private func syncSongsToDatabase(paths: Set<String>) {
        guard let projectDatabase else { return }
        let assets = songAssets.filter { paths.contains($0.relativePath) }
        guard !assets.isEmpty else { return }

        for asset in assets {
            suppressDatabaseSyncPaths.insert(asset.relativePath)
        }
        ...
        Task { [weak self] in
            ...
            for asset in assets {
                ...
                _ = await MainActor.run {
                    self.suppressDatabaseSyncPaths.remove(asset.relativePath)
                }
            }
