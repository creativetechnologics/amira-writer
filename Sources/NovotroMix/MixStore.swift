...
private struct MixSaveSnapshot {
    ...
    let connection: NovotroProjectConnection
    let document: MixProjectDocument
    let projectPath: String
    let backupData: Data?
    let backupPath: String?
    let backupProjectPath: String?
}
...
final class MixStore {
    ...
    private nonisolated static let mixActorID = NovotroProjectClientIdentity.actorID(for: "novotro-mix")
    nonisolated private static let audioExtensions: Set<String> = ["wav", "aiff", "aif", "mp3", "m4a", "caf", "flac"]
    nonisolated private static let databaseWatchInterval: TimeInterval = 0.45
    ...
    @ObservationIgnored private var connection: NovotroProjectConnection?
    ...
    @ObservationIgnored private var databaseWatchWorkItem: DispatchWorkItem?
    @ObservationIgnored private var databaseChangeToken: Int64 = 0
    ...
    func loadProject(url: URL, requestID: UInt64) async throws {
        ...
        let database = try await NovotroProjectConnection.open(projectURL: url, preferService: false)
        try ensureCurrentProjectLoad(requestID)
        try await database.ensureCurrentIndex()
        try ensureCurrentProjectLoad(requestID)
        let summary = try await database.loadProjectSummary()
        try ensureCurrentProjectLoad(requestID)
        let loadOutcome = try await loadStoredDocument(from: database)
        try ensureCurrentProjectLoad(requestID)
        let currentChangeToken = try await database.currentChangeToken()
        try ensureCurrentProjectLoad(requestID)

        connection = database
        projectURL = database.projectURL
        workingProjectURL = database.workingProjectURL
        databaseChangeToken = currentChangeToken
        applyProjectSummary(
            summary,
    ...
    private func loadStoredDocument(from connection: NovotroProjectConnection) async throws -> MixDocumentLoadOutcome {
        guard let record = try await connection.loadProjectFile(path: Self.mixProjectFile) else {
        ...
        do {
            return MixDocumentLoadOutcome(
                document: try decoder.decode(MixProjectDocument.self, from: record.jsonData),
                warningMessage: nil
            )
        } catch {
            let backupPath = Self.corruptBackupPath()
            let projectPath = connection.projectURL.standardizedFileURL.path
            do {
                try await connection.upsertProjectFile(
                    path: backupPath,
                    jsonData: record.jsonData,
                    actorID: Self.mixActorID
                )
            } catch {
                pendingCorruptDocumentBackup = record.jsonData
                pendingCorruptDocumentBackupPath = backupPath
                pendingCorruptDocumentBackupProjectPath = projectPath
                return MixDocumentLoadOutcome(
                    document: MixProjectDocument(),
                    warningMessage: "Recovered from an unreadable mix session file. Backup persistence will retry on the next save."
                )
            }
            return MixDocumentLoadOutcome(
                document: MixProjectDocument(),
    ...
    private func applyProjectSummary(
        ...
        // database polls (clearClipSelection: false) preserve the selection so a
    ...
    private func pollDatabaseChanges() {
        ...
        let currentProjectPath = projectURL?.standardizedFileURL.path

        Task { [weak self, connection, currentToken, currentProjectPath] in
            guard let self else { return }

            do {
                let changes = try await connection.listChanges(since: currentToken)
                guard !changes.isEmpty else { return }

                let shouldRefreshScenes = changes.contains {
                    $0.entityType == "scene" && $0.actorID != Self.mixActorID
                }
                let refreshedSummary = shouldRefreshScenes ? try await connection.loadProjectSummary() : nil
                let databaseProjectPath = connection.projectURL.standardizedFileURL.path

                await MainActor.run {
                    guard self.projectURL?.standardizedFileURL.path == currentProjectPath,
                          self.projectURL?.standardizedFileURL.path == databaseProjectPath else {
                        return
                    }

                    self.databaseChangeToken = changes.last?.changeID ?? self.databaseChangeToken

                    guard let refreshedSummary else { return }
                    self.applyProjectSummary(
                        refreshedSummary,
                        document: self.document,
                        preferredSelectionPath: self.selectedScene?.relativePath ?? self.document.lastSelectedScenePath,
                        clearClipSelection: false
                    )
                }
            } catch {
                NSLog("[MixStore] Database poll error: %@", error.localizedDescription)
            }
        }
    }
    ...
    private func makeSaveSnapshot() -> MixSaveSnapshot? {
        ...
        guard let projectPath = projectURL?.standardizedFileURL.path else { return nil }
        ...
        return MixSaveSnapshot(
            ...
            backupData: pendingCorruptDocumentBackupProjectPath == projectPath ? pendingCorruptDocumentBackup : nil,
            backupPath: pendingCorruptDocumentBackupPath,
            backupProjectPath: pendingCorruptDocumentBackupProjectPath
        )
    }

    private func performSave(_ snapshot: MixSaveSnapshot) async {
        do {
            if let backupData = snapshot.backupData,
               let backupPath = snapshot.backupPath,
               snapshot.backupProjectPath == snapshot.projectPath {
                try await snapshot.connection.upsertProjectFile(
                    path: backupPath,
                    jsonData: backupData,
                    actorID: Self.mixActorID
                )
            ...
            try await snapshot.connection.upsertProjectFile(
                path: Self.mixProjectFile,
                jsonData: data,
                actorID: Self.mixActorID
            )

            await MainActor.run {
                if self.pendingCorruptDocumentBackupProjectPath == snapshot.projectPath {
    ...
    /// Project-local paths are scanned first so they are included even if Desktop paths time out.
    ...
    nonisolated private static func candidateBrowserRoots(
        ...
        let desktopExport = desktop.appendingPathComponent("Novotro Score Export", isDirectory: true)
        let desktopPreviews = desktop.appendingPathComponent("Novotro Previews", isDirectory: true)
        let sunoDownloads = desktopExport.appendingPathComponent("Suno Downloads", isDirectory: true)
        let candidates = [
            desktopExport,
            desktopPreviews,
        ]

        for candidate in candidates where !Task.isCancelled && Self.pathExistsNoCloud(candidate.path) {
            roots.append(candidate)
        }

        if !Task.isCancelled,
           roots.contains(where: { $0.standardizedFileURL.path == desktopExport.standardizedFileURL.path }) == false,
           Self.pathExistsNoCloud(sunoDownloads.path) {
            roots.append(sunoDownloads)
        }
