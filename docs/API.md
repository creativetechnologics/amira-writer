...
| `GET` | `/automation/continuity-builder/session` | — | Load or create the guided Continuity Builder training session from project-local references and feedback artifacts. |
| `POST` | `/automation/continuity-builder/generate` | `{ "mode": "dry_run", "candidateCount": 3, "imageSize": "1K", "aspectRatio": "4:3", "maxCostUSD": 0.50 }` | Plan or execute cheap Continuity Builder candidate images. Paid Gemini/Vertex calls require `"mode":"execute"` plus `maxCostUSD`; outputs write sidecars and register with the image library for immediate Image Intelligence. |
| `POST` | `/automation/feedback/rules/extract` | `{ "mode": "dry_run", "model": "MiniMax-M2.7", "maxSources": 80, "write": true }` | Promote All Images notes and Continuity Builder feedback into canonical continuity rule fingerprints. `dry_run` uses local heuristics; `execute` uses MiniMax JSON extraction and writes `Metadata/automation/continuity-rules/latest-rules.json`. |
