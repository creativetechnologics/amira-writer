...
@main
struct NovotroProjectServiceMenuBarApp: App {
    @StateObject private var controller = ProjectServiceController()

    var body: some Scene {
        MenuBarExtra(
            "Novotro Project Server",
...
private final class ProjectServiceController: ObservableObject {
    ...
    @Published var port: UInt16 = NovotroProjectServiceConfiguration.defaultPort
    @Published var connectionCount = 0
    @Published var projects: [NPProjectServerRegistration] = []
    ...
    private var host: NovotroProjectServiceHost?

    init(registry: NovotroProjectServerRegistry = NovotroProjectServerRegistry()) {
    ...
    func addProject() {
        ...
        panel.title = "Add Project to Novotro Project Server"
        ...
        guard selectedURL.pathExtension.lowercased() == "owp" else {
            showError("Novotro Project Server can only import .owp project packages.")
            return
        }
    ...
    func removeProject(_ project: NPProjectServerRegistration) {
    ...
    func revealProject(_ project: NPProjectServerRegistration) {
    ...
    func copyManagedPath(_ project: NPProjectServerRegistration) {
    ...
    private func startServer() async {
        ...
        do {
            let host = try NovotroProjectServiceHost()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if controller.isExternallyManaged {
                Divider()
                Text("This menu bar app is attached to the background Project Server already running on this Mac.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
...
private final class ProjectServiceRelay {
    ...
    func confirmRemoval(_ displayName: String) -> Bool {
        ...
        alert.messageText = "Remove \(displayName) from Novotro Project Server?"
    ...
    func presentError(_ message: String) {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = "Novotro Project Server"
