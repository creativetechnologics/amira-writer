...
public struct NovotroProjectServiceInfo: Codable, Sendable {
    ...
    public var supportedOperations: [NovotroProjectServiceOperation]
    ...
    public init(
        ...
        startedAt: Date,
        supportedOperations: [NovotroProjectServiceOperation],
...
public struct NovotroProjectServiceMCPCapability: Codable, Sendable, Hashable {
...
public enum NovotroProjectServiceOperation: String, Codable, Sendable, CaseIterable {
    case ping
    case serviceInfo
    case mcpTools
...
public struct NovotroProjectServiceRequest: Codable, Sendable {
    public var operation: NovotroProjectServiceOperation
...
public struct NovotroProjectServiceResponse: Codable, Sendable {
    public var success: Bool
    public var errorMessage: String?
    public var serviceInfo: NovotroProjectServiceInfo?
    public var mcpTools: [NovotroProjectServiceMCPCapability]?
    ...
    public init(
        ...
        serviceInfo: NovotroProjectServiceInfo? = nil,
        mcpTools: [NovotroProjectServiceMCPCapability]? = nil,
...
struct NovotroProjectServiceEnvelope: Codable, Sendable {
    var version: Int
    var sealedPayload: Data

    init(version: Int = NovotroProjectTransportSecurity.currentVersion, sealedPayload: Data) {
        self.version = version
        self.sealedPayload = sealedPayload
    }
}

enum NovotroProjectTransportSecurity {
    static let currentVersion = 1
    private static let salt = Data("NovotroProjectServerTransport/v1".utf8)

    static func seal(_ payload: Data, authToken: String) throws -> NovotroProjectServiceEnvelope {
        let key = deriveKey(authToken: authToken)
        let sealedBox = try ChaChaPoly.seal(payload, using: key)
        return NovotroProjectServiceEnvelope(sealedPayload: sealedBox.combined)
    }

    static func open(_ envelope: NovotroProjectServiceEnvelope, authToken: String) throws -> Data {
        guard envelope.version == currentVersion else {
            throw NSError(
                domain: "NovotroProjectTransportSecurity",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Unsupported Novotro Project Server transport version \(envelope.version)."]
            )
        }
...
public enum NovotroProjectConnectionMode: String, Codable, Sendable {
    case local
    case remoteService
}

final class NovotroProjectOneShotContinuation<T: Sendable>: @unchecked Sendable {
