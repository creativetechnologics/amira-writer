...
public actor NovotroProjectConnection {
    private enum Backend: Sendable {
        case local(NovotroProjectDatabase)
        case mirrored(NovotroProjectMirrorSession)
        case remote(NovotroProjectRemoteClient)
    }

    public nonisolated let projectURL: URL
    public nonisolated let workingProjectURL: URL
    public nonisolated let mode: NovotroProjectConnectionMode

    private let backend: Backend

    private init(projectURL: URL, workingProjectURL: URL, mode: NovotroProjectConnectionMode, backend: Backend) {
    ...
    public static func open(projectURL: URL, preferService: Bool? = nil) async throws -> NovotroProjectConnection {
        ...
        if shouldUseService {
            await MainActor.run {
                NovotroProjectOpenProgressCenter.shared.start(
                    projectURL: normalizedURL,
                    phaseTitle: "Connecting To Project Server",
                    detail: "Checking the remote project service and preparing the local mirror."
                )
            }
            let client = try await NovotroProjectRemoteClient.discover(projectURL: normalizedURL)
            await MainActor.run {
                NovotroProjectOpenProgressCenter.shared.update(
                    projectURL: normalizedURL,
                    phaseTitle: "Preparing Local Mirror",
                    detail: "Inspecting any project files already cached on this Mac."
                )
            }
            let session = try await NovotroProjectMirrorSession.open(
                sourceProjectURL: normalizedURL,
                remoteClient: client
            )
            return NovotroProjectConnection(
                projectURL: normalizedURL,
        ...
        let database = NovotroProjectDatabase(
            projectURL: normalizedURL,
            databaseDirectoryURL: NovotroProjectClientIdentity.projectDatabaseDirectoryURL(for: normalizedURL)
        )
        return NovotroProjectConnection(
            projectURL: normalizedURL,
    ...
    public nonisolated static func shouldPreferRemoteService(for projectURL: URL) -> Bool {
        ...
        if environment["NOVOTRO_DISABLE_PROJECT_SERVICE"] == "1" {
            return false
        }
        if environment["NOVOTRO_FORCE_PROJECT_SERVICE"] == "1" {
            return true
        }
...
public actor NovotroProjectRemoteClient {
    ...
    public static func discover(projectURL: URL) async throws -> NovotroProjectRemoteClient {
        ...
        for endpoint in NovotroProjectServiceEndpointDiscovery.candidateEndpoints() {
            let client = NovotroProjectRemoteClient(endpoint: endpoint, projectURL: projectURL, authToken: authToken)
            do {
        ...
        throw lastError ?? NovotroProjectRemoteClientError.noCompatibleServiceFound(projectURL.lastPathComponent)
    }

    public static func connect(endpoint: NWEndpoint, projectURL: URL, authToken: String) -> NovotroProjectRemoteClient {
        NovotroProjectRemoteClient(endpoint: endpoint, projectURL: projectURL, authToken: authToken)
    }
    ...
    public func loadProjectSummary() async throws -> NPProjectSummary {
        ...
        guard let summary = response.projectSummary else {
            throw NovotroProjectRemoteClientError.invalidResponse("Missing project summary")
        }
        return summary
    }
    ...
    public func loadProject() async throws -> NPProjectRecord {
        ...
        guard let project = response.project else {
            throw NovotroProjectRemoteClientError.invalidResponse("Missing project record")
        }
        return project
    }
    ...
    public func summarizeProjectAssets() async throws -> NPProjectAssetSummary {
        ...
        guard let summary = response.projectAssetSummary else {
            throw NovotroProjectRemoteClientError.invalidResponse("Missing project asset summary")
        }
        return summary
    }
    ...
    private func send(_ request: NovotroProjectServiceRequest) async throws -> NovotroProjectServiceResponse {
        ...
        return try await withTaskCancellationHandler {
            ...
            let responseEnvelope: NovotroProjectServiceEnvelope
            do {
                responseEnvelope = try Self.decoder.decode(NovotroProjectServiceEnvelope.self, from: responseEnvelopeData)
            } catch {
                throw NovotroProjectRemoteClientError.invalidResponse(
            ...
            do {
                response = try Self.decoder.decode(NovotroProjectServiceResponse.self, from: responseData)
            } catch {
                throw NovotroProjectRemoteClientError.invalidResponse(
    ...
    private static func discoverBonjourEndpoints(timeout: TimeInterval = 2.5) async throws -> [NWEndpoint] {
        ...
        return try await withTaskCancellationHandler {
            return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[NWEndpoint], Error>) in
                ...
                let discovered = NovotroProjectEndpointAccumulator()
    ...
    static func pathsReferToSameProject(requestedProjectURL: URL, resolvedProjectPath: String) -> Bool {
        NovotroProjectPathIdentity.matches(
            requestedProjectURL: requestedProjectURL,
            resolvedProjectURL: NovotroProjectPathIdentity.normalizedProjectURL(from: resolvedProjectPath)
        )
    }
    ...
    static func responseAcceptsProjectIdentity(
        ...
        let requestedSignature = NovotroProjectPathIdentity.signature(
    ...
    private static func validateResolvedProjectPath(
        in response: NovotroProjectServiceResponse,
        for projectURL: URL,
        operation: NovotroProjectServiceOperation
    ) throws {
        guard operation != .ping else { return }
        guard let resolvedProjectPath = response.resolvedProjectPath else {
            throw NovotroProjectRemoteClientError.invalidResponse("Remote service did not return a resolved project path.")
        }
        guard responseAcceptsProjectIdentity(
            ...
            throw NovotroProjectRemoteClientError.projectResolutionMismatch(
...
private final class NovotroProjectEndpointAccumulator: @unchecked Sendable {
...
public enum NovotroProjectRemoteClientError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .discoveryTimedOut:
            return "Could not find Novotro Project Server on the local network."
        case let .invalidResponse(message):
            return message
        case let .remote(message):
            return message
        case .connectionCancelled:
            return "The connection to Novotro Project Server was cancelled."
        case .missingAuthToken:
            return "Novotro Project Server requires an access token, but no token is configured on this machine."
        case let .noCompatibleServiceFound(projectName):
            return "Found Novotro Project Server on the network, but none of the discovered servers matched \(projectName)."
        ...
        case let .projectResolutionMismatch(requested, resolved):
            return "Novotro Project Server resolved \(requested) to a different project path: \(resolved)"
        }
    }
}
