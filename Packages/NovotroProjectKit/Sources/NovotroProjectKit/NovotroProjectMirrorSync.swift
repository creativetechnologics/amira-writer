...
actor NovotroProjectMirrorSession {
    let sourceProjectURL: URL
    let workingProjectURL: URL
    let database: NovotroProjectDatabase

    private let remoteClient: NovotroProjectRemoteClient
    ...
    static func open(
        ...
        remoteClient: NovotroProjectRemoteClient
    ) async throws -> NovotroProjectMirrorSession {
        let workingProjectURL = NovotroProjectClientIdentity.mirrorProjectURL(
            for: sourceProjectURL
        )
        let session = NovotroProjectMirrorSession(
    ...
    init(
        ...
        self.clientID = NovotroProjectClientIdentity.sharedClientID()
        self.syncActorID = NovotroProjectClientIdentity.actorID(for: "novotro-sync")
        self.database = NovotroProjectDatabase(
            projectURL: self.workingProjectURL,
            databaseDirectoryURL: NovotroProjectClientIdentity.projectDatabaseDirectoryURL(for: sourceProjectURL)
        )
    }
        ...
        syncTask = Task {
            while Task.isCancelled == false {
                do {
                    ...
                    print("[NovotroMirrorSync] Background sync error: \(error)")
                }
    ...
    private func performDatabaseMutation(
        localMutation: @Sendable (NovotroProjectDatabase) async throws -> Void,
        remoteMutation: @Sendable (NovotroProjectRemoteClient) async throws -> Void
    ) async throws {
