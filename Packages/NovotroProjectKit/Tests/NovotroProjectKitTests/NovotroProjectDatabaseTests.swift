...
final class NovotroProjectDatabaseTests: XCTestCase {
    func testActiveVersionIDInvariantPrefersExplicitActiveVersion() async throws {
        ...
        let database = NovotroProjectDatabase(projectURL: fixture.projectURL)
    ...
    func testShouldPreferRemoteServiceHonorsEnvironmentOverrides() {
        ...
        unsetenv("NOVOTRO_DISABLE_PROJECT_SERVICE")
        XCTAssertFalse(NovotroProjectConnection.shouldPreferRemoteService(for: projectURL))

        setenv("NOVOTRO_FORCE_PROJECT_SERVICE", "1", 1)
        XCTAssertTrue(NovotroProjectConnection.shouldPreferRemoteService(for: projectURL))
...
private func makeFixtureProject() throws -> FixtureProject {
    ...
    let root = base
        ...
        .appendingPathComponent("NovotroProjectKitTests-\(UUID().uuidString)", isDirectory: true)
