...
final class ProjectServerClientIntegrationTests: XCTestCase {
    func testRemoteConnectionLoadsProjectWithoutInliningHeavyProjectFiles() async throws {
        ...
        let database = ProjectDatabase(projectURL: fixture)
        ...
        defer {
            host.stop()
            unsetenv(ProjectServiceEndpointDiscovery.endpointEnvironmentKey)
            unsetenv("PROJECT_SERVICE_FORCE")
            unsetenv(ProjectServiceConfiguration.authTokenEnvironmentKey)
        }
        await fulfillment(of: [ready], timeout: 5)

        setenv(ProjectServiceEndpointDiscovery.endpointEnvironmentKey, "127.0.0.1:\(port)", 1)
        setenv("PROJECT_SERVICE_FORCE", "1", 1)
        setenv(ProjectServiceConfiguration.authTokenEnvironmentKey, authToken, 1)

        let connection = try await ProjectConnection.open(projectURL: fixture)
    ...
    func testRemoteMutationRebuildsColdDatabaseBeforeWriting() async throws {
        ...
        let client = ProjectRemoteClient.connect(
    ...
    func testRemoteConnectionRejectsWrongAuthToken() async throws {
        ...
        setenv(ProjectServiceEndpointDiscovery.endpointEnvironmentKey, "127.0.0.1:\(port)", 1)
        setenv("PROJECT_SERVICE_FORCE", "1", 1)
        setenv(ProjectServiceConfiguration.authTokenEnvironmentKey, "wrong-token", 1)

        do {
            _ = try await ProjectConnection.open(projectURL: fixture)
    ...
    func testPathsReferToSameProjectAcceptsProgrammingMirrorPaths() {
        ...
        XCTAssertTrue(
            ProjectRemoteClient.pathsReferToSameProject(
    ...
    func testResponseAcceptsManagedProjectIdentityWhenRequestedSignatureIsRegisteredAlias() {
        ...
        let managedProjectURL = ProjectServerRegistry.defaultProjectsRootURL()
            .appendingPathComponent("Amira.owp", isDirectory: true)
        let resolvedManagedPath = managedProjectURL.path
        ...
        XCTAssertTrue(
            ProjectRemoteClient.responseAcceptsProjectIdentity(
...
) throws -> (ProjectServiceHost, UInt16) {
    for candidate in UInt16(21000)...UInt16(21032) {
        if let host = try? ProjectServiceHost(
            port: candidate,
            authToken: authToken,
...
private func makeServiceFixtureProject() throws -> URL {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("ProjectServiceTests-\(UUID().uuidString)", isDirectory: true)
