...
@MainActor
public final class ProjectServerBrowserModel: ObservableObject {
    @Published public private(set) var projects: [ProjectServerRegistration] = []
    ...
    public var selectedProject: ProjectServerRegistration? {
    ...
    public func refresh(selectStoredSelection: Bool = true) async {
        ...
        isLoading = true
        errorMessage = nil
        statusMessage = "Connecting to Project Server…"
        defer { isLoading = false }

        do {
            let client = try await ProjectServerClient.discover()
            let loadedProjects = try await ProjectAsyncTimeout.withTimeout(
                seconds: 6,
            ...
            statusMessage = loadedProjects.isEmpty
                ? "No projects are on Project Server yet."
                : "Loaded \(loadedProjects.count) server project\(loadedProjects.count == 1 ? "" : "s")."
        } catch {
            hasLoaded = false
            errorMessage = error.localizedDescription
            statusMessage = "Unable to reach Project Server."
        }
    }

    @discardableResult
    public func createProject(named displayName: String) async -> ProjectServerRegistration? {
        await mutate(
    ...
    @discardableResult
    public func renameSelectedProject(to displayName: String) async -> ProjectServerRegistration? {
    ...
    public func removeSelectedProject(deleteManagedProject: Bool = true) async {
        ...
        _ = await mutate(
            ...
            self.statusMessage = "Removed \(selectedProject.displayName) from Project Server."
            return true
        }
    }

    public func noteOpenedProject(_ project: ProjectServerRegistration) {
        selectedProjectID = project.id
        userDefaults.set(project.id.uuidString, forKey: lastOpenedProjectKey)
    }

    public func preferredOpenURL(for project: ProjectServerRegistration) -> URL {
    ...
    public func beginOpeningProject(_ project: ProjectServerRegistration, in appName: String) -> Bool {
    ...
    public static func preferredOpenURL(
        for project: ProjectServerRegistration,
        fileManager: FileManager = .default
    ) -> URL {
        var candidates: [URL] = []
    ...
    private static func url(forAliasSignature signature: String) -> URL? {
        ...
        // Skip Documents/ aliases to avoid TCC permission prompts.
        // Projects are no longer stored in ~/Documents.
...
public struct ProjectServerBrowserView: View {
    @Environment(\.dismiss) private var dismiss
    @ObservedObject private var model: ProjectServerBrowserModel
    ...
    public init(
        model: ProjectServerBrowserModel,
        appName: String,
    ...
    private var actionRow: some View {
        HStack(spacing: 10) {
            ...
            .buttonStyle(.bordered)
            .help("Refresh projects from Project Server")
            .disabled(model.isLoading || model.isMutating || model.isOpeningProject)
        }
    }
                ...
                if let selectedProject = model.selectedProject {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Text(selectedProject.sourceProjectURL == nil
                             ? "This project lives fully inside Project Server and can be opened from Write, Score, or Animate."
                             : "This project is mirrored into Project Server and stays available to the full Opera suite.")
                            .font(.system(size: 13, weight: .regular, design: .monospaced))
    ...
    private func loadingPanel(for project: ProjectServerRegistration) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            Text(snapshot?.phaseTitle ?? "Opening Project")
                ...
                .tracking(1.2)
                .foregroundStyle(Color.white.opacity(0.46))
            Text(snapshot?.detail ?? "Loading \(project.displayName) from Project Server. This can take a moment for larger projects.")
                .font(.system(size: 13, weight: .regular, design: .monospaced))
                .foregroundStyle(Color.white.opacity(0.72))
