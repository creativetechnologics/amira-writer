...
public struct OperaChromeSplitHandle: View {
    ...
    @State private var dragStartPrimary: CGFloat?
    ...
    public var body: some View {
        ...
        .frame(
            maxWidth: axis == .vertical ? .infinity : thickness,
            maxHeight: axis == .horizontal ? .infinity : thickness
        )
        .contentShape(Rectangle())
        .onHover { hovering in
            isHovering = hovering
        ...
        .gesture(
            DragGesture(minimumDistance: 0, coordinateSpace: .global)
                .onChanged { value in
                    let primary = axis == .horizontal ? value.location.x : value.location.y
                    if dragStartPrimary == nil {
                        dragStartPrimary = primary
                        lastTranslation = 0
                    }
                    let delta = primary - dragStartPrimary!
