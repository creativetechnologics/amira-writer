...
final class TempoLaneView: NSView {
    ...
    private func barIndex(at x: CGFloat) -> Int? {
        ...
        for i in sorted.indices {
            ...
            if x >= x0 && x < x1 {
                // Map back to the unsorted index in tempoEvents
                let matchTick = sorted[i].tick
                let matchBPM = sorted[i].bpm
                return tempoEvents.firstIndex(where: { $0.tick == matchTick && $0.bpm == matchBPM })
            }
        }
        return nil
    }
    ...
    override func mouseDown(with event: NSEvent) {
        guard currentTool == .paintbrush else { return }
        lastPaintedTick = -1
    ...
    override func mouseUp(with event: NSEvent) {
        lastPaintedTick = -1
        frozenRange = nil
        // Redraw with dynamic range now that painting is done
        setNeedsDisplay(bounds)
    }
