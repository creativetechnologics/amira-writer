...
final class LLMAnimationPlanCompilerTests: XCTestCase {
    ...
    func testCompilerValidatesInvalidObjectAttachmentTarget() {
        let compiler = LLMAnimationPlanCompiler()
        let plan = LLMAnimationPlan(
            sceneName: "Bad Attachment",
            objectPlacements: [
                LLMObjectPlacement(
                    objectName: "lantern",
                    frame: 0,
                    position: .init(x: 0.5, y: 0.5),
                    attachmentTarget: "character:"
                )
            ]
        )
        ...
        XCTAssertFalse(report.isValid)
        XCTAssertTrue(report.issues.contains(where: { $0.code == .invalidAttachmentTarget }))
    }

    func testCompilerEmitsDetachCueForClearAttachmentDirective() {
        let compiler = LLMAnimationPlanCompiler()
        let plan = LLMAnimationPlan(
            sceneName: "Detach Prop",
            objectPlacements: [
                LLMObjectPlacement(
        ...
        let compiled = compiler.compile(plan, fps: 24)
        let track = compiled.tracks["object:medical-satchel:action"]
        let expressions = track?.compactMap { keyframe -> String? in
            if case let .expression(name) = keyframe.value {
                return name
            }
            return nil
        } ?? []

        XCTAssertEqual(expressions, ["attach:character:Luke:hand_right", "attach:none"])
    }

    func testAttachmentReferenceParsesObjectAndWorldTargets() {
        let objectAttachment = ObjectAttachmentReference.parse("object:tea-tray:top")
        XCTAssertEqual(objectAttachment?.kind, .object)
        XCTAssertEqual(objectAttachment?.targetName, "tea-tray")
        XCTAssertEqual(objectAttachment?.anchor, "top")

        let worldAttachment = ObjectAttachmentReference.parse("world:center_floor")
        XCTAssertEqual(worldAttachment?.kind, .world)
        XCTAssertEqual(worldAttachment?.targetName, "center_floor")
        XCTAssertNil(worldAttachment?.anchor)
    }

    func testCompilerAcceptsWorldAndObjectAttachmentTargets() {
        let compiler = LLMAnimationPlanCompiler()
        let plan = LLMAnimationPlan(
            sceneName: "Attachment Variants",
            objectPlacements: [
                LLMObjectPlacement(
                    objectName: "tea-tray",
                    frame: 0,
                    position: .init(x: 0.52, y: 0.78),
                    attachmentTarget: "world:center_floor"
                ),
                LLMObjectPlacement(
                    objectName: "teacup",
                    frame: 0,
                    position: .init(x: 0.52, y: 0.72),
                    attachmentTarget: "object:tea-tray:top"
                )
            ]
        )
        ...
        let compiled = compiler.compile(plan, fps: 24)
        let trayTrack = compiled.tracks["object:tea-tray:action"]
        let cupTrack = compiled.tracks["object:teacup:action"]

        let trayExpressions = trayTrack?.compactMap { keyframe -> String? in
            if case let .expression(name) = keyframe.value { return name }
            return nil
        } ?? []
        let cupExpressions = cupTrack?.compactMap { keyframe -> String? in
            if case let .expression(name) = keyframe.value { return name }
            return nil
        } ?? []

        XCTAssertEqual(trayExpressions, ["attach:world:center_floor"])
        XCTAssertEqual(cupExpressions, ["attach:object:tea-tray:top"])
    }
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("AnimateTests-\(UUID().uuidString)")
