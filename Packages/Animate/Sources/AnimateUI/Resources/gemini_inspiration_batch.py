...
# PIL/Image is loaded via the bootstrap machinery below (after helper defs)
# so it can auto-install Pillow into the per-user vendor dir when the
# bundled python doesn't have it.
...
def _pip_install_to_vendor(package: str) -> None:
    python = sys.executable or "python3"
...
def _load_google_genai_modules() -> tuple[object, object]:
    ...
    try:
        _pip_install_to_vendor("google-genai")
...
def _load_pil_image() -> object:
    ...
    try:
        _pip_install_to_vendor("Pillow")
    except Exception as error:
        raise RuntimeError(
            "Unable to install the required Pillow package automatically. "
            "Please run `python3 -m pip install --user Pillow` and try again."
        ) from error
    ...
    try:
        ...
        raise RuntimeError(
            "The Pillow package was installed but could not be imported by the batch helper."
        ) from error
