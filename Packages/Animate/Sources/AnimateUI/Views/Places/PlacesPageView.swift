...
struct PlacesPageView: View {
    ...
    private var overviewSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(spacing: 10) {
                PlacesOverviewPill(title: "Total Places", value: "\(store.backgrounds.count)", systemImage: "map")
                PlacesOverviewPill(title: "Photo Outputs", value: "\(store.backgrounds.reduce(0) { $0 + $1.imagePaths.count })", systemImage: "camera")
                PlacesOverviewPill(title: "Animated Outputs", value: "\(store.backgrounds.reduce(0) { $0 + $1.animatedImagePaths.count })", systemImage: "paintpalette")
                PlacesOverviewPill(title: "Landmark Refs", value: "\(store.placesWorkflowLibrary.landmarkReferences.count)", systemImage: "square.stack.3d.up")
            }
    ...
    private var worldMapSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 10) {
                        PlacesOverviewPill(title: "Pinned Images", value: "\(worldbuildingSnapshot.totalPinnedCaptures)", systemImage: "photo.badge.location")
                        PlacesOverviewPill(title: "Nodes", value: "\(worldbuildingSnapshot.nodes.count)", systemImage: "scope")
                        PlacesOverviewPill(title: "Flags", value: "\(worldbuildingSnapshot.totalFlaggedReviews)", systemImage: "exclamationmark.triangle")
                        PlacesOverviewPill(title: "Unplaced", value: "\(worldbuildingSnapshot.unplacedCaptureCount)", systemImage: "mappin.slash")
                        PlacesOverviewPill(title: "Homeless", value: "\(worldbuildingSnapshot.homelessCaptureCount)", systemImage: "house.slash")
                    }
                    .padding(.vertical, 2)
                }
...
// MARK: - Reference Card
// PlaceGridCard moved to PlaceGridCard.swift.
// PlaceLandmarkDetailView moved to PlaceLandmarkDetailView.swift.
// PlaceLandmarkProfileCard moved to PlaceLandmarkProfileCard.swift.
// PlaceReferenceThumbnailCard moved to PlaceReferenceThumbnailCard.swift.

// MARK: - Angle Image Card

// AngleImageCard moved to AngleImageCard.swift.
