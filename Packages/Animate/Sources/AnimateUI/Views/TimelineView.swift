...
final class TimelineEditorView: NSView {
    ...
    private func updatePlayhead(at xPosition: CGFloat) {
        guard let store else { return }
        let labelWidth = renderer?.labelWidth ?? 120
        let frame = Int((xPosition - labelWidth + scrollOffset.x) / pixelsPerFrame)
